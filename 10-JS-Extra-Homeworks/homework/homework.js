...
function deObjetoAmatriz(objeto) {
  ...
  var arregloDeObjeto = []
    ...
    for (let prop in objeto) {
      arregloDeObjeto.push([prop, objeto[prop]])
    }
...
function capToFront(s) {
  ...
  var unirArreglos = [...mayus,...minus]
  return unirArreglos.join('');
}
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  //necesitamos seperar las palabras
  // could use .reverse() for each word
  var nuevoDato = [];

  str.split(' ').forEach((item) => {
    nuevoDato.push(item.split('').reverse().join(''))
  })
  return nuevoDato.join(' ')
} 
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
  //.filter()
  var nuevaCadena = cadena.split('').filter((letra) => {
